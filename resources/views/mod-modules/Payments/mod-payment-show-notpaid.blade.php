...
<div class="card shadow mb-4">
	<div class="card-header py-2">
		...
		@endforeach
		<h6 class="m-0 font-weight-bold text-primary d-inline-block py-2">
			Polizas por pagar del vendedor: {{ $policy->user->name.' '.$policy->user->lastname }}
		</h6>
	</div>
	<div class="card-body">
			...
			<form action="/mod/selected-pay/{{$not_paid[0]->user->id}}" method="POST">
				...
				<table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0"
					...
					<tbody>
						...
						@foreach($not_paid as $policy)
						<tr>
							<input type="hidden" value="{{$policy->user->name.' '.$policy->user->lastname}}"
								name="name">
							<td>{{$policy->id}}</td>
							...
							@if(!$policy->statusu)
							@if($policy->report)
							<td><input type="checkbox" checked='checked' class="settings" name="update_checkbox[]"
									value="{{$policy->id}}"></td>
							@else
							<td><input type="checkbox" class="settings" name="update_checkbox[]"
									value="{{$policy->id}}"></td>
							@endif
							@else
							<td>
								<p class="text-danger">ANULADA</p>
							</td>
							@endif
						...
						<div class="modal fade" id="deleteModal-{{$policy->id}}" tabindex="-1" role="dialog"
							...
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									<div class="modal-body"><span>La primera poliza creada no se puede Eliminar por
											aqui, para eliminarla debe ir a polizas</div>
									<div class="modal-footer">
										<form action="" method="POST">
											@csrf
											<a href="/mod/index-policy/{{$policy->id}}"
												class="btn btn-info float-right">Ver Poliza</a>
						...
						{{-- Borrar Poliza --}}
						<div class="modal fade" id="deleteModal1-{{$policy->id}}" tabindex="-1" role="dialog"
							aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">¿Seguro que desea <strong
												class="text-danger">eliminar</strong> esta poliza?</h5>
										<button class="close" type="button" data-dismiss="modal" aria-label="Close">
											<span aria-hidden="true">×</span>
										</button>
									</div>
									<div class="modal-body">Seleccione "continuar" si desea <span
											class="text-danger">eliminar</span> esta poliza</div>
									<div class="modal-footer">
