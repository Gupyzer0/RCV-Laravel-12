...
<div class="card shadow mb-4">
	<div class="card-header py-3">
		...
		<a href="{{route('user.index.spolicies')}}" class="float-right btn btn-danger text-white">X</a>

	</div>
	<div class="card-body">

		<form action="{{ route('user.register.policy.submit')}}" method="POST" enctype="multipart/form-data" id="form_policies">
			@csrf
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel"> <strong class="text-danger">Pocos Contratos</strong></h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <div class="modal-body text-info"></div>
        <p style="font-size: 20px; text-align: center;" class="text-danger">Cantidad de contratos: {{$cant}}</p>
...
$(document).ready(function() {
    ...
    let userConfirmed = false; // Bandera para rastrear la confirmación

    // Al seleccionar "Sí"...
    trailerSelect.on('change', function() {
        if ($(this).val() === '1') {
            userConfirmed = false; // Reinicia la bandera
    ...
    confirmBtn.on('click', function() {
        userConfirmed = true; // Marca como confirmado
        trailerModal.modal('hide');
    });

    // Al cerrar el modal (solo resetea si NO hubo confirmación)...
    trailerModal.on('hidden.bs.modal', function() {
        if (!userConfirmed) {
            trailerSelect.val(''); // Resetea solo si el usuario canceló
        }
    });
});
</script>
